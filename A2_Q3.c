...
int main()
{
   ...
   double mul[3][3];
   double mul2[3];
   //declaring the size of m matrix by dynamic memory allocting
   double** M=malloc(3*sizeof(double*)); 
   for(i=0;i<3;++i){
	M[i]=malloc(3*sizeof(double));	
   }
   //declaring the size of n matrix by dynamic memory allocting
   ...
   //extracing data of N  from N.txt file
   FILE *file2;
   file2=fopen("N.txt", "r");
   for(i = 0; i < 3; i++){
      for(j = 0; j < 3; j++) {
